...
export default function Toast({ type, message, show, onClose }) {
  ...
  const { bg, text, iconBg, iconText, iconPath } =
  ...
  return (
    <div
      role="alert"
      className={`flex items-center w-full max-w-xs p-4 mb-4 ${text} ${bg} rounded-lg shadow-sm`}
    >
      <div
        className={`inline-flex items-center justify-center shrink-0 w-8 h-8 ${iconText} ${iconBg} rounded-lg`}
      >
        <svg
          className="w-5 h-5"
          aria-hidden="true"
